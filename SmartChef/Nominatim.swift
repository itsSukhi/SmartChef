...
class Nominatim {
  class func getLocation(fromAddress address: String, completion: @escaping (_ error: Error?, _ result: Location?) -> Void)  {
    ...
    let queryURL =  URL(string:"https://maps.googleapis.com/maps/api/geocode/json?" + address.addingPercentEncoding(withAllowedCharacters: .urlHostAllowed)! + "?format=json&addressdetails=1&limit=1")!
  ...
  class func getLocation(fromLatitude latitude: String, longitude: String, completion: @escaping (_ error: Error?, _ result: Location?) -> Void)  {
    ...
    let queryURL =  URL(string:"https://maps.googleapis.com/maps/api/geocode/json?&lat=" + latitude + "&lon=" + longitude + "&addressdetails=1&limit=1")!
    let session = URLSession.shared
