...
class rejectReqViewController: UIViewController {
    ...
    @IBAction func rejectButton(_ sender: Any) {
        if Reachability.isConnectedToNetwork() {
            ...
            anotherQueue.async{
                self.rejectChatRequest(sessionTime: self.sessionTime, requestId: self.recRequestId, id: self.id){ (success) in
                    if success{
...
extension rejectReqViewController{
    
    func rejectChatRequest(sessionTime : String ,requestId : String, id: String, completion: @escaping (_ success: Bool) -> Void){
        SVProgressHUD.show()
        ...
        print("PARAMETERS Of rejectChatRequest :\(parameters)")
        var request = URLRequest(url: URL(string:  URLConstants().BASE_URL + URLConstants().REJECT_REQUEST)!)
        request.setValue(Authorization, forHTTPHeaderField: "Authorization")
        let url = URL(string:  URLConstants().BASE_URL + URLConstants().REJECT_REQUEST)!
